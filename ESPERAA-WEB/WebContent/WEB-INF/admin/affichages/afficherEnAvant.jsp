...
<c:if test="${!empty listeEnAvant}">
	<p class="text-center">
		<span class="glyphicon glyphicon-star" aria-hidden="true"></span> <Strong>Chevaux
			en vedette</Strong> <span class="glyphicon glyphicon-star" aria-hidden="true"></span>
	</p>
					...
					<c:forEach var="cheval" items="${listeEnAvant}">
						<c:if test="${number==0}">
							<li data-target="#carousel-example-generic"
								data-slide-to="${number}" class="active"></li>
						</c:if>
						<c:if test="${number!=0}">
							<li data-target="#carousel-example-generic"
					...
					<c:forEach var="cheval" items="${listeEnAvant}">
						<c:if test="${number==0}">
							<div class="item active">
								...
								<a href="<c:out value="${afficherCheval}"/>"> <img
									class="slide-image"
									src="http://www.larepublique77.fr/files/2014/12/Unknown-1.jpg"
									alt="">
								</a>
							</div>
						</c:if>
